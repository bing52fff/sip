#include "arp.h"

struct arpt_arp		arp_table[ARP_TABLE_SIZE];

void init_arp_entry()
{
	int i = 0;
	for (i = 0; i < ARP_TABLE_SIZE; ++ i)
	{
		arp_table[i].ctime = 0;
		memset(arp_table[i].ethaddr, 0, ETH_ALEN);
		arp_table[i].ipaddr = 0;
		arp_table[i].status = ARP_EMPTY;
	}
}
struct arpt_arp* arp_find_entry(__u32 ip)
{
	int i;
	struct arpt_arp *found = NULL;
	for (i = 0; i < ARP_TABLE_SIZE; ++ i)
	{
		if (time(NULL) > arp_table[i].ctime + ARP_LIVE_TIME)
		{
			arp_table[i].status = ARP_EMPTY;
		}
		else if (arp_table[i].ipaddr == ip && arp_table[i].status == ARP_ESTABLISHED)
		{
			found = &arp_table[i];
			break;
		}
	}
	return found;
}
struct arpt_arp* update_arp_entry(__u32 ip, __u8 *ethaddr)
{
	struct arpt_arp *found = arp_find_entry(ip);
	if (found)
	{
		memcpy(found->ethaddr, ethaddr, ETH_ALEN);
		found->status = ARP_ESTABLISHED;
		found->ctime = time(NULL);
	}
	return found;
}
void arp_add_entry(__u32 ip, __u8 *ethaddr, int status)
{
	int i = 0;
	struct arpt_arp *found = update_arp_entry(ip, ethaddr);
	if (!found)
	{
		for (i = 0; i < ARP_TABLE_SIZE; ++ i)
		{
			if (arp_table[i].status == ARP_EMPTY)
			{
				found = &arp_table[i];
				break;
			}
		}
	}
	if (found)
	{
		found->ipaddr = ip;
		memcpy(found->ethaddr, ethaddr, ETH_ALEN);
		found->status = status;
		found->ctime = time(NULL);
	}
}
struct sk_buff* arp_create(struct net_device *dev,
		int type,
		__u32 src_ip,
		__u32 dest_ip,
		__u8 *src_hw,
		__u8 *dest_hw,
		__u8 *target_hw)
{
	struct sk_buff *skb;
	struct sip_arphdr *arph;
	skb = skb_alloc(ETH_ZLEN);
	if (!skb)
		goto out;
	skb->pyh.raw = skb_put(skb, sizeof(struct sip_ethhdr));
	skb->nh.raw = skb_put(skb, sizeof(struct sip_arphdr));

	arph = skb->nh.arph;
	skb->dev = dev;
	if (src_hw == NULL)
		src_hw = dev->hwaddr;
	if (dest_hw == NULL)
		dest_hw = dev->hwbroadcast;
	skb->pyh.ethh->h_proto = htons(ETH_P_ARP);
	memcpy(skb->pyh.ethh->h_dest, dest_hw, ETH_ALEN);
	memcpy(skb->pyh.ethh->h_source, src_hw, ETH_ALEN);

	arph->ar_op = htons(type);
	arph->ar_hrd = htons(ETH_P_802_3);
	arph->ar_pro = htons(ETH_P_IP);
	arph->ar_hln = ETH_ALEN;
	arph->ar_pln = 4;
	memcpy(arph->ar_sha, src_hw, ETH_ALEN);
	memcpy(arph->ar_sip, (__u8*)&src_ip, 4);
	memcpy(arph->ar_tip, (__u8*)&dest_ip, 4);
	if (target_hw != NULL)
		memcpy(arph->ar_tha, target_hw, ETH_ALEN);
	else
		memset(arph->ar_tha, 0xff, dev->hwaddr_len);
out:
	return skb;
}
int arp_input(struct sk_buff **pskb, struct net_device *dev)
{
	struct sk_buff *skb = *pskb;
	__be32 ip = 0;
	int retval = 0;
	if (skb->tot_len < sizeof(struct sip_arphdr))
	{
		retval = -1;
		goto out;
	}
	ip = *(__be32*)(skb->nh.arph->ar_tip);
	if (ip == dev->ip_host.s_addr)
	{
		update_arp_entry(ip, dev->hwaddr);
	}
	struct in_addr t_addr;
	switch (ntohs(skb->nh.arph->ar_op))
	{
	case ARPOP_REQUEST:
		t_addr.s_addr = *(__u32*) skb->nh.arph->ar_sip;
		debug_print("recive arp request");
		arp_send(dev, ARPOP_REPLY, dev->ip_host.s_addr, *(__u32*)skb->nh.arph->ar_sip, dev->hwaddr, skb->pyh.ethh->h_source, skb->nh.arph->ar_sha);
		arp_add_entry(*(__u32*)skb->nh.arph->ar_sip, skb->pyh.ethh->h_source, ARP_ESTABLISHED);
		break;
	case ARPOP_REPLY:
		arp_add_entry(*(__u32*)skb->nh.arph->ar_sip, skb->pyh.ethh->h_source, ARP_ESTABLISHED);
		break;
	default:
		break;
	}
out:
	return retval;
}
void arp_send(struct net_device *dev,
		int type,
		__u32 src_ip,
		__u32 dest_ip,
		__u8 *src_hw,
		__u8 *dest_hw,
		__u8 *target_hw)
{
	struct sk_buff *skb;
	skb = arp_create(dev, type, src_ip, dest_ip, src_hw, dest_hw, target_hw);
	if (skb)
		dev->linkoutput(skb, dev);
}
int arp_request(struct net_device *dev, __u32 ip)
{
	struct sk_buff *skb;
	__u32	tip;
	if ((ip & dev->ip_netmask.s_addr) == (dev->ip_host.s_addr & dev->ip_netmask.s_addr))
		tip = ip;
	else
		tip = dev->ip_gw.s_addr;
	skb = arp_create(dev, ARPOP_REQUEST, dev->ip_host.s_addr, tip, dev->hwaddr, NULL, NULL);
	if (skb)
		dev->linkoutput(skb, dev);
}
